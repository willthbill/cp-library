// https://www.spoj.com/problems/SUBMERGE/ - articulation points 
    ...
    void _dfs(int node, int d) {
        ...
        for(int& nb : adj[node]) {
            if(depth[nb] != -1) { // node is visited, its a back-edge
                ...
                if(d != 0 && d <= low_link[nb]) {
                    is_art_point = 1;
                }
            }
        }
        if(d == 0 && cnt_fwd_edges >= 2) { // d = 0 means root in subgraph
            is_art_point = 1;
        }
        if(is_art_point) {
