// https://www.hackerearth.com/practice/algorithms/graphs/articulation-points-and-bridges/tutorial/ -- both
...
void testcase(__attribute__((unused)) signed tc){
    ...
    auto arts = util.get_art_points();
    auto bridges = util.get_bridges();
    ...
    prl(bridges.size());
    foe(e, bridges) {
        prl(min(e.fi,e.se), max(e.fi, e.se));
    }
}
